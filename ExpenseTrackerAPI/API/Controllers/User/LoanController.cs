...
public class LoansController : ControllerBase
{
    ...
    [HttpPut("{id}")]
    public async Task<IActionResult> UpdateLoan(int id, [FromBody] LoanUpdateRequest request)
    {
        try
        {
            await _loanService.UpdateLoanAsync(id, request, GetUserId());
            return Ok(new { Message = "Khoản vay được cập nhật thành công" });
        }
        catch (Exception ex)
        {
            return BadRequest(new { Message = ex.Message });
